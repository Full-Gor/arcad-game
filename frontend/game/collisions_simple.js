...
export function checkCollisions(geodesicShield = null) {
    if (!starship) return;
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (checkLaserCollision(starship)) {
            ...
            // Vérifier si le bouclier géodésique est actif (remplace le bouclier doré)
            if (geodesicShield && geodesicShield.isActive) {
                console.log('🛡️ Bouclier géodésique actif ! Laser absorbé !');
                geodesicShield.createImpact(starship.x + starship.width/2, starship.y + starship.height/2);
            } else {
        ...
        if (checkCollision(starship, enemy)) {
            ...
            // NOUVEAU: Vérifier si le bouclier géodésique est actif (remplace le bouclier doré)
            if (geodesicShield && geodesicShield.isActive) {
                console.log('🛡️ Bouclier géodésique actif ! Collision absorbée !');
                geodesicShield.createImpact(enemy.x + enemy.width/2, enemy.y + enemy.height/2);
    ...
    for (let i = enemyBullets.length - 1; i >= 0; i--) {
        ...
        // Sinon, bouclier géodésique (remplace le bouclier doré)
        else {
            const geodesicActive = geodesicShield && geodesicShield.isActive;
            console.log('🔍 DEBUG: geodesicActive =', geodesicActive);
            
            if (geodesicActive) {
                // Vérifier collision avec le bouclier géodésique
                if (geodesicShield.checkCollision(enemyBullet.x, enemyBullet.y, 3)) {
                    console.log('🛡️ Bouclier géodésique ! Projectile absorbé !');
                    geodesicShield.createImpact(enemyBullet.x, enemyBullet.y);
                    collisionDetected = true;
                }
            } else {
                // Pas de bouclier → collision directe avec le joueur
                const bulletRect = { x: enemyBullet.x - 2, y: enemyBullet.y - 2, width: 4, height: 4 };
                if (checkCollision(starship, bulletRect)) {
                    collisionDetected = true;
                }
            }
        }
        
        // Ancien système de bouclier sphérique désactivé en version beta
